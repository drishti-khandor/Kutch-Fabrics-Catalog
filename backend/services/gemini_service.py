...
def build_model_prompt(product_name: str, categories: list[str]) -> str:
    ...
    bg = (
        "Background: a richly atmospheric Indian heritage setting — choose one of: "
        "(a) an ornate Rajasthani haveli courtyard with carved stone arches, decorative jali screens, "
        "warm sandstone columns, and terracotta-tiled floor; lush potted plants in the corners; "
        "soft warm golden-hour sunlight streaming through the archways; "
        "(b) a palace veranda with majestic stone pillars framing a lush garden and a historic fort "
        "visible in the soft-focus background; ornate mosaic-tiled floor; warm late-afternoon light; "
        "a few rolled fabric bolts propped nearby; "
        "(c) a warm traditional haveli interior with intricately carved stone arches, brass/copper "
        "vessels on the floor, antique wooden seating with printed cushions, "
        "and warm ambient lamp-light casting a golden glow across carved surfaces; "
        "(d) an artisan textile weaving studio with rustic wooden shelves stacked with neatly folded "
        "handloom fabrics, a loom in the background, and soft natural window light. "
        "Lighting is warm, golden, and natural — never harsh or clinical. "
        "Skin looks luminous and real. The garment's texture and craftsmanship are clearly visible. "
    )
    pose = (
    ...
    if is_saree:
        return (
            ...
            "• PALLU: In the reference photo, identify the pallu as the end section (~18–24 inches) "
            "that is VISUALLY DISTINCT from the body fabric — it may be plain/solid where the body "
            "is printed, or have a unique border/design. That contrasting section IS the pallu. "
            "Reproduce it exactly, pinned at the LEFT shoulder cascading visibly down the FRONT-LEFT. "
            "NEVER replace it with the body fabric pattern. NEVER hide it behind the model.\n"
            "• BODY FABRIC ZONE (everything except the pallu cascade): The continuous repeat pattern "
            "of the main fabric covers the torso wrap, ALL front pleats, and the full hem to the floor. "
            "The body fabric pattern is uniform and consistent throughout this entire zone.\n"
            "• The front pleats show the body fabric pattern ONLY.\n"
            "• The hem at the feet shows the body fabric pattern ONLY.\n"
            "\n\nCRITICAL RULE — ZERO TOLERANCE: The pallu's decorative design (denser motifs, "
            "special border, end-section pattern) MUST appear ONLY on the pallu cascade from the "
            "left shoulder. It must NEVER appear on the pleats, torso wrap, hem, or any other part "
            "of the saree body. If you are unsure whether a design element belongs to the pallu or "
            "the body fabric — use the plain body fabric pattern there instead.\n"
